...
public class CounselDao {
    public ArrayList<Counsel> selectSubjectList(Connection conn, String counselSubject, int start,
            int end) {
        ...
        if (counselSubject.equals("전체")) {
            query = "select * from(" + "(select rownum as rnum, n.* from("
                    + "select * from health_counsel_tbl " + "order by counsel_no desc)n)) "
                    + "where rnum between ? and ?";
        } else {
            query = "select * from(" + "(select rownum as rnum, n.* from("
                    + "select * from health_counsel_tbl " + "where counsel_subject = ? "
                    + "order by counsel_no desc)n)) " + "where rnum between ? and ?";
        }
    ...
    public int selectSubjectListCount(Connection conn, String counselSubject) {
        ...
        if (counselSubject.equals("전체")) {
            ...
            query = "select count(*) as cnt from health_counsel_tbl " + "where counsel_subject = ?";
        }
    ...
    public ArrayList<Counsel> titleSearch(Connection conn, String searchWord, int start, int end) {
        ...
        String query = "select * from(" + "(select rownum as rnum, n.* from("
                + "select * from health_counsel_tbl " + "where counsel_title like ? "
                + "order by counsel_no desc)n)) " + "where rnum between ? and ?";
    ...
    public int titleSearchCount(Connection conn, String searchWord) {
        ...
        int totalCount = 0;
        String query =
                "select count(*) as cnt from health_counsel_tbl " + "where counsel_title like ?";
    ...
    public int insertCounsel(Connection conn, Counsel c) {
        ...
        int result = 0;
        String query =
                "insert into health_counsel_tbl values(?, counsel_seq.nextval, ?, ?, 0, to_char(SYSDATE, 'YYYY-MM-DD'), 0, ?, ?, ?)";
    ...
    public int updateCounselCount(Connection conn, int counselNo) {
        ...
        int result = 0;
        String query =
                "update health_counsel_tbl set counsel_count = counsel_count+1 where counsel_no = ?";
    ...
    public int insertComment(Connection conn, CounselComment cc) {
        PreparedStatement pstmt = null;
        int result = 0;
        String query = "insert into counsel_com_tbl " + "values(counsel_com_seq.nextval, "
    ...
    public int modifyCounsel(Connection conn, Counsel c) {
        ...
        String query = "update health_counsel_tbl set counsel_title = ?, "
                + "counsel_content = ?, filename = ?, " + "filepath = ?, counsel_subject = ?"
                + "where counsel_no = ?";
    ...
    public int updateComment(Connection conn, CounselComment cc) {
        ...
        int result = 0;
        String query =
                "update counsel_com_tbl " + "set comment_content = ? " + "where comment_no = ?";
    ...
    public ArrayList<Counsel> getMyQuestion(Connection conn, String memberId, int startNum,
            int endNum) {
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        ...
        String query =
                "SELECT R.* FROM (SELECT ROWNUM RNUM, COUNSEL_NO, COUNSEL_TITLE, COUNSEL_DATE, COUNSEL_ANSWER, COUNSEL_SUBJECT FROM (SELECT * FROM HEALTH_COUNSEL_TBL WHERE MEMBER_ID=? ORDER BY COUNSEL_NO DESC))R WHERE RNUM BETWEEN ? AND ?";
        try {
            pstmt = conn.prepareStatement(query);
    ...
    public ArrayList<Counsel> getMyAnswer(Connection conn, int memberNo, int startNum, int endNum) {
        ...
        ArrayList<Counsel> clist = new ArrayList<Counsel>();
        String query =
                "SELECT RNUM, COUNSEL_TITLE, COUNSEL_SUBJECT, COUNSEL_DATE, COUNSEL_NO FROM (SELECT ROWNUM AS RNUM, R.* "
