...
public class QnaDao {
    ...
    public ArrayList<Qna> selectQnaList(Connection conn, int start, int end) {
        ...
        // 10개씩 짤라오면서 댓글가져오기
        String query =
                "select * from (select rownum as rnum,n.*from (select * from HEALTH_INFO_TBL join (select info_no,(select count(*) from health_info_com_tbl ic where ic.info_no=info.info_no ) as commentCount from health_info_tbl info)a using (info_no) order by INFO_NO desc)n)where rnum between ? and ?";
        // "select*from (select rownum as rnum,n.*from(select * from HEALTH_INFO_TBL order by
        // INFO_NO desc)n)where rnum between ? and ?";
        // "select info_no, info_title,info_content,info_count,(select count(*) from
        // health_info_com_tbl ic where ic.info_no=info.info_no ) as commentCount from
        // health_info_tbl info";
        try {
    ...
    public int insertQna(Connection conn, Qna q) {
        ...
        int result = 0;
        String query =
                "insert into HEALTH_INFO_TBL values(?,HEALTH_INFO_SEQ.nextval, ?,?, 0,to_char(sysdate,'yyyy-mm-dd'),?,?) ";
    ...
    public int updateQna(Connection conn, Qna q) {
        ...
        int result = 0;
        String query =
                "update HEALTH_INFO_TBL set info_title=?, info_content=?, filename=?, filepath=? where info_no=?";
        try {
            pstmt = conn.prepareStatement(query);
    ...
    public int insertQnaComment(Connection conn, QnaComment qc) {
        ...
        int result = 0;
        String query =
                "insert into HEALTH_INFO_COM_TBL values(?,?,HEALTH_INFO_COM_SEQ.nextval,?,to_char(sysdate,'yyyy-mm-dd'))";
