...
public class ClinicDao {
    public ArrayList<Doctor> selectAllDoctor(Connection conn, String doctorMajor) {
        ...
        String query =
                "SELECT * FROM doctor_tbl JOIN member_tbl ON (doctor_no = member_no) where doctor_major = ?";
    ...
    public Doctor selectOneDoctor(Connection conn, int doctorNo) {
        ...
        Doctor doctor = null;
        String query =
                "SELECT * FROM doctor_tbl JOIN member_tbl ON (doctor_no = member_no) where doctor_no = ?";
        try {
            pstmt = conn.prepareStatement(query);
    ...
    public int insertReserve(Connection conn, Book book) {
        ...
        String query =
                "insert into book_tbl values(?, BOOK_TBL_SEQ.nextval, ?, ?, ?, ?, 1, 0, null, ?, ?, ?)";
        try {
            pstmt = conn.prepareStatement(query);
    ...
    public ArrayList<MyClinic> getMyReserve(Connection conn, int memberNo, int startNum,
            int endNum) {
        // select rownum, book_no, hospital_name, doctor_major, book_date, member_state from
        // book_tbl join doctor_tbl on (doctor_no=BOOK_DOCTOR) where member_no=117 and rownum
        // between 1 and 3 order by book_no desc;
        PreparedStatement pstmt = null;
        ResultSet rset = null;
    ...
    public MyClinic getMyClinicDetail(Connection conn, int bookNo) {
        ...
        String query =
                "SELECT B.BOOK_NO, B.MEMBER_STATE, B.BOOK_PHARM_NO, B.BOOK_DATE, B.BOOK_HISTORY, B.BOOK_PRICE, B.HOSPITAL_NAME, B.HOSPITAL_ADDR, "
    ...
    public ArrayList<MyClinic> getMyReserve(Connection conn, int memberNo) {
        ...
        ArrayList<MyClinic> rlist = new ArrayList<MyClinic>();
        String query =
                "SELECT * FROM (SELECT B.MEMBER_NO, B.BOOK_NO, B.BOOK_DATE, B.MEMBER_STATE, M.MAJOR, M.MEMBER_NAME AS DOCTOR_NAME, B.BOOK_TIME "
                        + "FROM BOOK_TBL B "
    ...
    public String doctorName(Connection conn, String bookDoctor) {
        ...
        String query =
                "select member_name from member_tbl join book_tbl on (member_id = book_doctor) where member_id = ?";
